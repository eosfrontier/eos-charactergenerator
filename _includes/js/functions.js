...
function SH_nicknameForm(charID, sheetID) {
    ...
    $.ajax({
        ...
        data: {
            nickNameForm: postdata
        }
    }

    ).done(function (data) {
        SH_animateFormDiv(data);
        /* show the response */
    }

    ).fail(function () {
        SH_animateFormDiv("<h4><i class=\"fas fa-warning\"></i>&nbsp;Posting failed. [err 702]</h4>");
...
function SH_editPlayedForm(charID) {
    ...
    $.ajax({
        ...
        SH_animateFormDiv("<h4><i class=\"fas fa-warning\"></i>&nbsp;Posting failed. [err 703]</h4>");
        /* just in case posting your form failed */
    }
